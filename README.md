...
| Session Report | Active Docker product surface | Summarizes runtime state, recent activity, proof sources, recovery drills, and stronger-proof blockers in one Docker-safe customer report. |
...
| Docker session report | Active Docker proof gate | Verifies `/api/session-report` exposes a customer-readable Docker-safe runtime report without promoting stronger proof claims. |
