...
| Proof Promotion Center | Active Docker product surface | Shows which Docker-local claims are ready to describe and which Docker daemon, VM/ISO, live OAuth, browser, release, mutation, and attestation claims still require observed evidence. |
...
| Docker proof promotion center | Active Docker proof gate | Verifies `/api/proof-promotion` exposes claim promotion decisions, required evidence, source surfaces, and non-claims without automatic promotion. |
