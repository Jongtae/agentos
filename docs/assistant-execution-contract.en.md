...
- **Admissible public lookup (every route).** A worker whose context holds private material may still propose `web_search`, `weather`, `bounded_public_research` or `public_page_read`, or it may propose one in the same tool batch as a private-store write. AgentOS then composes the outbound arguments itself from that proposal. It keeps only words taken from text permitted for this lookup (`lookup_sources`):
  - the owner's current request;
  - earlier owner messages whose Work read or wrote no private store (inherited history taint and a CLI's unmediated reads concern the reply, not what the owner typed);
  - earlier replies whose Work saw only owner conversation.

  AgentOS never keeps a word this Work wrote to a private store (Memory candidates, notes), including writes proposed in the same batch. Matching is exact for digits and otherwise allows only a short particle. The worker can therefore select permitted words but cannot add one, and its redaction is not trusted. A place name must be the owner's own wording; an AI-composed transliteration or country code is not sent. When nothing permitted remains, the worker gets a question to ask and nothing is sent. The checked arguments are exactly the transmitted arguments, recorded as `sent` with a count of excluded words, never the words themselves. The Work binding is rechecked before each lookup: the Work must still be running with its original request row. The package/revision recheck still applies. Budget: one network attempt per destination per Work, recorded before the request, so a repeat after failure or success gets the first outcome. Delegated specialists never compose a lookup. No extra model or per-search approval is involved.
...
- **Not yet provided.** No disclosure-approval flow exists for a genuinely necessary sensitive transmission; such a lookup is refused with a question instead. Pending/resume disclosure approvals must reuse the existing approval target-match and expiry. A lookup that runs in an earlier model step than a Memory write in the same Work is not retroactively covered. There is no saved-location registry. Korean place names are sent to the geocoder as the owner wrote them; live geocoding of such names has not been observed.
...
- **Rollback.** `egress_composition = {"mode": "restrictive"}` disables admissible composition and treats every shown earlier message as history-tainted. That is stricter than either route before #605.
