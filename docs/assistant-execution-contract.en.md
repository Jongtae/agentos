...
- **Not yet provided, and residuals.** No disclosure-approval flow exists for a genuinely necessary sensitive transmission; such a lookup is refused with a question instead. Pending/resume disclosure approvals must reuse the existing approval target-match and expiry. There is no saved-location registry. Korean place names are sent to the geocoder as the owner wrote them; live geocoding of such names has not been observed. Recorded residuals:
  ...
  - *Ordering.* A lookup in an earlier model step than a private-store write of the same Work is not retroactively covered by the exclusion; the current-message judgment covers the value when it is in the current message.
  ...
  - *Rollback.* In restrictive mode a clean context is still composed (judgment and place wording); a private context is refused.
...
- **Rollback.** `egress_composition = {"mode": "restrictive"}` refuses a lookup from a private context instead of composing it, and treats every shown earlier message as history-tainted. That is stricter than either route before #605.
