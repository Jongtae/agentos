...
- **Admissible public lookup (every route, every lookup).** Every `web_search`, `weather` and `bounded_public_research` call of a Work with a lookup resolver is composed by AgentOS from the worker's proposal, the first turn included; `public_page_read` is composed when the context holds private material. The composition:
  - never sends a word this Work wrote to a private store (Memory candidates, notes, calendar drafts, and writes proposed in the same tool batch, in either call order). Values are compared after NFKC and casefolding, and a word whose digit run is part of a written value's digit run with separators removed is withheld, so `M1234-5678`, `M1234 5678`, `5678` and case or full-width variants of a saved `M12345678` do not leave;
  - sends words of the owner's *current* message only after the existing DecisionEngine judgment path (`ConversationJudgments.lookup_term_sensitivity`, the #597 seam: typed binary envelope, the one owner utterance and the candidate terms, the same route and audit) did not name them as sensitive. It is asked only when a lookup would include current-message words: once for the set and, after a yes, once per term. No engine, an unavailable, abstaining (below the policy threshold), malformed or contradictory answer fails safe: every term carrying a digit is withheld and ordinary words still go out. No new model, cue list or regex decides sensitivity;
  - requires a place name to be the owner's own wording on every path (clean contexts too); an AI-composed transliteration or country code is not sent;
  - when the context holds private material, keeps only words taken from text permitted for this lookup (`lookup_sources`): earlier owner messages whose Work read or wrote no private store (a durable successful write event labels its Work with the store's label, whichever process ran it), earlier replies whose Work saw only owner conversation, and the owner's current request. A word must equal a permitted word or be a prefix of one; a word that extends a permitted word sends only the permitted word, so `병원이혼` sends `병원`. Digit runs must be identical. Words are deduplicated, put in the order they have in permitted text (message by message; the worker's order is not kept) and capped at 8. One network attempt per destination per Work is recorded as a durable `requested` tool event *before* the request, so a repeat after failure or success, a restarted bridge or a resumed Work gets no second attempt.

  A clean context keeps the worker's other words (its own knowledge, such as a translation), because no private material is in that context. When nothing admissible remains, or a place name is not the owner's wording, the worker gets a question to ask and nothing is sent. The checked arguments are exactly the transmitted arguments, recorded as `sent` with a count of withheld words, never the words themselves. The Work binding is rechecked before each lookup, and the package/revision recheck still applies. Delegated specialists compose the same way, share the parent's written values, and never compose from a private context. No per-search approval is involved.
...
- **Not yet provided, and residuals.** No disclosure-approval flow exists for a genuinely necessary sensitive transmission; such a lookup is refused with a question instead. Pending/resume disclosure approvals must reuse the existing approval target-match and expiry. There is no saved-location registry. Korean place names are sent to the geocoder as the owner wrote them; live geocoding of such names has not been observed. Recorded residuals:
  ...
  - *Sensitivity judgment scope.* Only words of the current message are judged, and only words that match it (a translation of a sensitive word is not recognised as current-message content). Sensitive words of an *earlier* owner message whose Work wrote no private store remain permitted text. With the engine unavailable, non-digit sensitive words of the current message (for example a diagnosis) are not withheld.
