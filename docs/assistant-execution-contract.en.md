...
- **Separate public task (direct-API route).** When a worker whose context holds private material proposes `web_search`, `weather`, `public_page_read` or `bounded_public_research`, its arguments are discarded. AgentOS runs a fresh `run_agent` whose only inputs are the host-captured owner request for this Work (rechecked: the Work is still running with its original request row) and exactly that one tool; the tool's package/revision recheck still applies. One public task per destination per Work. Its observed result returns as untrusted public evidence marked `composed_by: agentos-public-task`. A request that does not itself name what to look up yields a question, not a guess. Delegated specialists never get a public task.
- **CLI routes.** No separate public task. A worker session holding private material is refused with the actual source named; AgentOS's own preflight of an explicit `/search` request still runs from the raw request only.
- **Rollback.** `egress_composition = {"mode": "restrictive"}` disables the public task and treats every shown earlier message as history-tainted. That is stricter than either route before #605.
- **Not owned.** Under `trusted-local` the CLI can read host files outside AgentOS provenance, so #605 cannot guarantee what that worker composes from those reads. OS-level read isolation is #616.
