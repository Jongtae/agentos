...
**Threat scope (owner decision, 2026-09-26, [#605](https://github.com/Jongtae/agentos/issues/605)).** In scope and deterministic: a saved or withheld value never leaves in any spelling or format variant — any Unicode decimal digits, case and width variants, split or re-joined words, separators — on every path (clean, private, no judgment, explicit `/search`, weather, bridge). Accepted residual, bounded and documented below: deliberate covert encoding by a hostile worker in a clean context.
...
- **Admissible public lookup (every route, every lookup).** Every `web_search`, `weather` and `bounded_public_research` call of a Work with a lookup resolver is composed by AgentOS from the worker's proposal, the first turn included; `public_page_read` is composed when the context holds private material. The composition:
  ...
  - **explicit `/search <query>` (owner decision D1).** When the owner's current message is `/search <query>` and the worker proposes exactly that query (whitespace-normalised), that typed string is sent for that one lookup without the sensitivity judgment, minus saved private values. Worker-rewritten or added words are not covered and go through the normal composition. This applies on the direct-API route and to the CLI route's host-side preflight;
...
- **Not yet provided, and residuals.** No disclosure-approval flow exists for a genuinely necessary sensitive transmission; such a lookup is refused with a question instead. Pending/resume disclosure approvals must reuse the existing approval target-match and expiry. There is no saved-location registry. Korean place names are sent to the geocoder as the owner wrote them; live geocoding of such names has not been observed. Recorded residuals:
  - *Covert encoding by a hostile worker (accepted residual, owner threat scope 2026-09-26).* In a clean context the worker writes its own query words. A worker that deliberately encodes information in which words it chooses, their order and up to 12 allowlisted punctuation characters cannot be stopped while worker-authored queries are allowed; AgentOS guarantees deterministically only that no saved or withheld value leaves in any spelling or format. The channel is bounded, not closed: at most 12 distinct words per lookup (about 13 bits each from a 10,000-word vocabulary, plus about 29 bits of order) and at most 12 operator characters at chosen positions (about 7 bits each), roughly 280 bits per lookup, and at most 6 clean-context lookups per Work, roughly 1.7 kbit per Work. This applies only to a deliberately hostile worker; it is not a spelling-variant leak.
