...
- **Admissible public lookup (every route, every lookup).** Every `web_search`, `weather` and `bounded_public_research` call of a Work with a lookup resolver is composed by AgentOS from the worker's proposal, the first turn included; `public_page_read` is composed when the context holds private material. The composition:
  ...
  The Work binding is rechecked after the judgment and before the request, so a Work cancelled while the judgment runs sends nothing. A clean context sends the worker's own string (its own knowledge, such as a translation) with only the withheld tokens removed, so punctuation such as `3.13`, `C++20`, `site:`, `-deno` or `₩50,000` is kept; a private context sends the permitted words in AgentOS's order. When nothing admissible remains, or a place name is not admissible, the worker gets a question to ask and nothing is sent. The checked arguments are exactly the transmitted arguments, recorded as `sent` with a count of withheld words, never the words themselves. The package/revision recheck still applies. Delegated specialists compose the same way, share the parent's written values and judgment cache, and never compose from a private context. No per-search approval is involved.
