...
- **Admissible public lookup (every route, every lookup).** Every `web_search`, `weather` and `bounded_public_research` call of a Work with a lookup resolver is composed by AgentOS from the worker's proposal, the first turn included; `public_page_read` is composed when the context holds private material. The composition:
  ...
  - judges the lookup when it includes **any** content of the owner's current message. This is decided inclusively: every outbound word counts unless it is taken from an *earlier* permitted text and not from the current message, so a reformatted, split, spelled-out or translated value counts. The current message and the **whole** outbound term list (capped at 12 terms; terms beyond the cap are never sent) go once to the existing DecisionEngine judgment path (`ConversationJudgments.lookup_term_sensitivity`, the #597 seam, via the `choose_many` typed envelope over index labels: the one owner utterance and the term list, the same route and audit). The engine returns the terms to withhold. The result is cached per (current message, term list) within the Work. With no engine, or an unavailable, abstaining (below the policy threshold), malformed or contradictory answer, **no current-message content is sent**; words taken from earlier permitted text may still go out. No new model, cue list or regex decides sensitivity. Without a configured DecisionEngine, a lookup therefore carries only words from earlier permitted messages;
  - in a clean context, a weather place may be the worker's transliteration with a two-letter ASCII country code (validated in form and upper-cased; anything else is omitted), and both pass the same judgment. In a private context the place and country must be the owner's own wording (F4.1);
  ...
  The Work binding is rechecked after the judgment and before the request, so a Work cancelled while the judgment runs sends nothing. A clean context keeps the worker's other words (its own knowledge, such as a translation), subject to the same exclusion and judgment. When nothing admissible remains, or a place name is not admissible, the worker gets a question to ask and nothing is sent. The checked arguments are exactly the transmitted arguments, recorded as `sent` with a count of withheld words, never the words themselves. The package/revision recheck still applies. Delegated specialists compose the same way, share the parent's written values and judgment cache, and never compose from a private context. No per-search approval is involved.
...
- **Not yet provided, and residuals.** No disclosure-approval flow exists for a genuinely necessary sensitive transmission; such a lookup is refused with a question instead. Pending/resume disclosure approvals must reuse the existing approval target-match and expiry. There is no saved-location registry. Korean place names are sent to the geocoder as the owner wrote them; live geocoding of such names has not been observed. Recorded residuals:
  ...
  - *Judgment route.* The Jev route offers no multi-selection and answers unavailable, which withholds current-message content. The CLI bridge builds an `AgentService` over the same store for the judgment on first use; with a subscription-CLI decision route that runs the decision CLI from inside the bridge process. Decision-audit appends are one immediate transaction, safe across the two processes.
