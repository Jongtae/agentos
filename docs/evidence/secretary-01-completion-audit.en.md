...
| 6 | **Probes A, B, C and the held-out scenario** complete on the owner's installation with evidence recorded. | None. Fixture runs do not satisfy this item. The recorder and runbook exist (PR #660). | Records `secretary-01-probe-<A|B|C|D>-<UTC yyyy-mm-ddTHHMMZ>-<work8>.json` from the owner's installation on the closeout head, each with the owner's check below. Probe B and the preparation part of probe C depend on SEC-ATTN-01 #659. | `pending owner run` |
