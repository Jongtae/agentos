...
- `docker-observed-proof-request-board-epic` — [EPIC: Stage 3 / Phase 2 Docker observed proof request board](https://github.com/Jongtae/agentos/issues/287)
  ...
  - Exit condition: complete after `/api/proof-requests`, the browser Observed Proof Request Board panel, README, TASKS, Docker acceptance, roadmap state, focused proof-request gate, Product Layer completion gate, runtime preview Python smoke, compose config, cleanup policy, and CI checks all preserve evidence-request, redaction, validation, and proof-promotion boundaries without stronger proof claims.
  - Open issue: #287.
  - Active task: P2-121 ([#288](https://github.com/Jongtae/agentos/issues/288)).
