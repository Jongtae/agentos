...
| --- | --- |
| GOV-EXEC-02 / [#410](https://github.com/Jongtae/personal-agentos/issues/410) | Complete on merge after required CI/review: one mutable GitHub execution cursor per active top-level program, cursor-first delta resume, anomaly-only full reconciliation, and a thin Claude bootstrap. Cursor state is cache-only and cannot replace canonical issue/PR/CI/review evidence. |
