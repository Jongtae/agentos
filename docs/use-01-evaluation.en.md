...
The current candidate validation observed 351 pytest tests with 231 subtests and 316 unittest tests, plus canonical-document/source-layout/AgentPackage checks, root/package delivery-plan equality, ledger JSON parsing and `git diff --check`. Live provider quality, real browser observation and owner acceptance remain pending and are reported separately.
...
2. Declare the exact provider endpoint as an owner-approved destination, plus the data class, any allowed public URLs, request/token or cost ceiling and timeout. Keep local document sharing disabled unless the owner explicitly approves that exact destination and scope. For the runner, pass the endpoint as one of the `--destination` values together with `--provider`, `--endpoint`, `--model`, `--data-class`, `--budget`, and `--timeout`; omit `--execute-live` for a configuration-only dry run.
3. After separately authorizing execution, set the API key through an environment reference (never in the command or report) and add `--api-key-env NAME --execute-live`. The bounded transport enforces the declared deadline, approved endpoint scope and maximum request count; this command prepares an authorized run but does not claim evaluation results until cases actually execute. Stop and report if the provider does not expose a trustworthy cost boundary.
