...
OPENAI_API_KEY=dummy python3 src/main.py --no-tui --workspace "$WORKSPACE_DIR" <<'EOS' > "$OUTPUT_FILE"
2
list files in this directory
exit
EOS

if ! rg -q "AgentOS Kernel Engine Setup" "$OUTPUT_FILE"; then
  echo "[demo] missing first-boot setup banner"
  cat "$OUTPUT_FILE"
  exit 1
fi

if ! rg -q "Selected kernel engine: codex" "$OUTPUT_FILE"; then
  echo "[demo] missing selected engine output"
  cat "$OUTPUT_FILE"
  exit 1
fi

if ! rg -q "ai>" "$OUTPUT_FILE"; then
  echo "[demo] missing ai prompt"
  cat "$OUTPUT_FILE"
  exit 1
fi

if ! rg -q "AI:" "$OUTPUT_FILE"; then
  echo "[demo] missing AI response"
  cat "$OUTPUT_FILE"
  exit 1
fi

echo "demo boot flow: PASS"
