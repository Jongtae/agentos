...
OUT_JSON="$TMP_DIR/broader-preview-cohort-operations.json"
python3 scripts/kernel_broader_preview_cohort_operations.py \
  --workspace "$WORKSPACE" \
...
python3 scripts/kernel_broader_preview_cohort_operations.py --validate "$OUT_JSON"

python3 - "$OUT_JSON" <<'PY'
import json, pathlib, sys
payload = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
assert payload["schema_version"] == "agentos-broader-preview-cohort-operations.v1"
assert payload["summary"]["delivery_scope"] == "broader_preview_candidate"
assert payload["summary"]["cohort_mode"] == "broader_preview_operator_guided"
assert payload["updated_broader_preview_launch_pack"]["schema_version"] == "agentos-updated-broader-preview-launch-pack.v1"
assert pathlib.Path(payload["artifacts"]["broader_preview_cohort_operations_json"]).exists()
print("kernel broader preview cohort operations smoke: PASS")
PY
