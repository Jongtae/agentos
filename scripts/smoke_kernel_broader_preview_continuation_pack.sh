...
TMP_DIR="/tmp/agentos-broader-cont-$$"
rm -rf "$TMP_DIR"
mkdir -p "$TMP_DIR"
...
OUT_JSON="$TMP_DIR/broader-preview-continuation-pack.json"
python3 scripts/kernel_broader_preview_continuation_pack.py \
  --workspace "$WORKSPACE" \
...
python3 scripts/kernel_broader_preview_continuation_pack.py --validate "$OUT_JSON"

python3 - "$OUT_JSON" <<'PY'
import json, pathlib, sys
payload = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
assert payload["schema_version"] == "agentos-broader-preview-continuation-pack.v1"
assert payload["summary"]["operating_health"] == "watch"
...
print("kernel broader preview continuation pack smoke: PASS")
PY
