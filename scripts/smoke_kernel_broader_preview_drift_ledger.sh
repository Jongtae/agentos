...
TMP_DIR="/tmp/agentos-broader-drift-$$"
rm -rf "$TMP_DIR"
mkdir -p "$TMP_DIR"
...
OUT_JSON="$TMP_DIR/broader-preview-drift-ledger.json"
python3 scripts/kernel_broader_preview_drift_ledger.py \
  --workspace "$WORKSPACE" \
  --feedback-file "$ARTIFACTS/feedback.json" \
  --report-dir "$ARTIFACTS/public-preview" \
  --snapshot-label c \
...
python3 scripts/kernel_broader_preview_drift_ledger.py --validate "$OUT_JSON"

python3 - "$OUT_JSON" <<'PY'
import json, pathlib, sys
payload = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
assert payload["schema_version"] == "agentos-broader-preview-drift-ledger.v1"
assert payload["current_signal_snapshot"]["schema_version"] == "agentos-broader-preview-signal-snapshot.v1"
assert payload["summary"]["drift_state"] in {"baseline", "improving", "steady", "regressing"}
print("kernel broader preview drift ledger smoke: PASS")
PY
