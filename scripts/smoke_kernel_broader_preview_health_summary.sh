...
TMP_DIR="/tmp/agentos-broader-health-$$"
rm -rf "$TMP_DIR"
mkdir -p "$TMP_DIR"
...
OUT_JSON="$TMP_DIR/broader-preview-health-summary.json"
python3 scripts/kernel_broader_preview_health_summary.py \
  --workspace "$WORKSPACE" \
...
python3 scripts/kernel_broader_preview_health_summary.py --validate "$OUT_JSON"

python3 - "$OUT_JSON" <<'PY'
import json, pathlib, sys
payload = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
assert payload["schema_version"] == "agentos-broader-preview-health-summary.v1"
assert payload["summary"]["operating_health"] == "watch"
assert payload["summary"]["operating_recommendation"] == "continue_with_caution"
assert payload["broader_preview_issue_ledger_manifest"]["schema_version"] == "agentos-broader-preview-issue-ledger.v1"
print("kernel broader preview health summary smoke: PASS")
PY
