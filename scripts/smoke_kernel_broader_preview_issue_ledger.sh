...
OUT_JSON="$TMP_DIR/broader-preview-issue-ledger.json"
python3 scripts/kernel_broader_preview_issue_ledger.py \
  --workspace "$WORKSPACE" \
...
python3 scripts/kernel_broader_preview_issue_ledger.py --validate "$OUT_JSON"

python3 - "$OUT_JSON" <<'PY'
import json, pathlib, sys
payload = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
assert payload["schema_version"] == "agentos-broader-preview-issue-ledger.v1"
assert payload["summary"]["issue_counts"]["pause"] >= 1
assert payload["summary"]["operating_decision"] == "pause_risk_present"
assert payload["feedback_triage_manifest"]["schema_version"] == "agentos-feedback-triage.v1"
print("kernel broader preview issue ledger smoke: PASS")
PY
