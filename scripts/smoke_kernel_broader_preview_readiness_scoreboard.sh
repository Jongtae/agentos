...
mkdir -p "$ARTIFACTS/kernel-policy" "$ARTIFACTS/validation-history" "$ARTIFACTS/public-preview/broader-preview-readiness-scoreboard/iteration/limited-preview-iteration-ledger"
...
cat >"$ARTIFACTS/public-preview/broader-preview-readiness-scoreboard/iteration/limited-preview-iteration-ledger/latest-limited-preview-iteration-ledger.json" <<'EOF'
{"snapshot_label":"previous","summary":{"current_watch_items":["Recovery wording"]}}
EOF

OUT_JSON="$TMP_DIR/broader-preview-readiness-scoreboard.json"
python3 "$ROOT_DIR/scripts/kernel_broader_preview_readiness_scoreboard.py" \
  --workspace "$WORKSPACE" \
...
python3 "$ROOT_DIR/scripts/kernel_broader_preview_readiness_scoreboard.py" --validate "$OUT_JSON"

python3 - "$OUT_JSON" <<'PY'
import json, pathlib, sys
payload = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding='utf-8'))
assert payload["schema_version"] == "agentos-broader-preview-readiness-scoreboard.v1"
...
assert "recovery_clarity" in payload["summary"]["blocked_reasons"]
assert pathlib.Path(payload["artifacts"]["broader_preview_readiness_scoreboard_manifest_json"]).exists()
print("kernel broader preview readiness scoreboard smoke: PASS")
PY
