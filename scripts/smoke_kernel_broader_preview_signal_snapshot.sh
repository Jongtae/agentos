...
TMP_DIR="/tmp/agentos-broader-signal-$$"
rm -rf "$TMP_DIR"
mkdir -p "$TMP_DIR"
...
OUT_JSON="$TMP_DIR/broader-preview-signal-snapshot.json"
...
python3 scripts/kernel_broader_preview_signal_snapshot.py --validate "$OUT_JSON"

python3 - "$OUT_JSON" <<'PY'
import json, pathlib, sys
payload = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
assert payload["schema_version"] == "agentos-broader-preview-signal-snapshot.v1"
...
assert payload["public_preview_position_update"]["schema_version"] == "agentos-public-preview-position-update.v1"
print("kernel broader preview signal snapshot smoke: PASS")
PY
