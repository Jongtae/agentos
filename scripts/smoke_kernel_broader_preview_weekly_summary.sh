...
TMP_DIR="/tmp/agentos-broader-weekly-$$"
rm -rf "$TMP_DIR"
mkdir -p "$TMP_DIR"
...
python3 scripts/kernel_broader_preview_signal_snapshot.py \
  --workspace "$WORKSPACE" \
  --feedback-file "$ARTIFACTS/feedback.json" \
  --report-dir "$ARTIFACTS/public-preview" \
  --snapshot-label b >/dev/null

OUT_JSON="$TMP_DIR/broader-preview-weekly-summary.json"
python3 scripts/kernel_broader_preview_weekly_summary.py \
  --workspace "$WORKSPACE" \
  --feedback-file "$ARTIFACTS/feedback.json" \
  --report-dir "$ARTIFACTS/public-preview" \
  --snapshot-label c \
...
python3 scripts/kernel_broader_preview_weekly_summary.py --validate "$OUT_JSON"

python3 - "$OUT_JSON" <<'PY'
import json, pathlib, sys
payload = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
assert payload["schema_version"] == "agentos-broader-preview-weekly-summary.v1"
assert payload["broader_preview_health_summary"]["schema_version"] == "agentos-broader-preview-health-summary.v1"
assert payload["broader_preview_drift_ledger"]["schema_version"] == "agentos-broader-preview-drift-ledger.v1"
assert payload["summary"]["weekly_posture"] in {"watch", "stable"}
print("kernel broader preview weekly summary smoke: PASS")
PY
