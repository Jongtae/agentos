...
REPORT_JSON="$TMP_DIR/external-eval.json"
python3 "$ROOT_DIR/scripts/kernel_external_evaluation_checklist.py" \
  --workspace "$WORKSPACE" \
...
python3 "$ROOT_DIR/scripts/kernel_external_evaluation_checklist.py" --validate "$REPORT_JSON"

python3 - "$REPORT_JSON" <<'PY'
import json, pathlib, sys
payload = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
assert payload["schema_version"] == "agentos-external-evaluation-checklist.v1"
assert payload["summary"]["recommendation"] == "hold"
assert pathlib.Path(payload["artifacts"]["external_evaluation_checklist_markdown"]).exists()
assert pathlib.Path(payload["artifacts"]["feedback_intake_manifest_json"]).exists()
print("kernel external evaluation checklist smoke: PASS")
PY
