...
REPORT_JSON="$TMP_DIR/candidate.json"
python3 "$ROOT_DIR/scripts/kernel_preview_candidate.py" \
  --workspace "$WORKSPACE" \
...
python3 "$ROOT_DIR/scripts/kernel_preview_candidate.py" --validate "$REPORT_JSON"

python3 - "$REPORT_JSON" <<'PY'
import json, pathlib, sys
payload = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
assert payload["schema_version"] == "agentos-preview-candidate.v1"
...
assert pathlib.Path(payload["artifacts"]["preview_candidate_manifest_json"]).exists()
print("kernel preview candidate smoke: PASS")
PY
