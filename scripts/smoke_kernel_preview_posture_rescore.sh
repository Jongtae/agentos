...
OUT_JSON="$TMP_DIR/preview-posture-rescore.json"
python3 scripts/kernel_preview_posture_rescore.py \
  --workspace "$WORKSPACE" \
...
python3 scripts/kernel_preview_posture_rescore.py --validate "$OUT_JSON"

python3 - "$OUT_JSON" <<'PY'
import json, pathlib, sys
payload = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding='utf-8'))
assert payload["schema_version"] == "agentos-preview-posture-rescore.v1"
assert payload["summary"]["candidate_state"] in {"candidate_watch", "broader_preview_candidate"}
assert payload["broader_preview_readiness_manifest"]["schema_version"] == "agentos-broader-preview-readiness-scoreboard.v1"
assert payload["recovery_watch_reevaluation_manifest"]["schema_version"] == "agentos-recovery-watch-reevaluation.v1"
assert pathlib.Path(payload["artifacts"]["preview_posture_rescore_manifest_json"]).exists()
print("kernel preview posture rescore smoke: PASS")
PY
