...
REPORT_JSON="$TMP_DIR/rehearsal.json"
python3 "$ROOT_DIR/scripts/kernel_preview_rehearsal_loop.py" \
  --workspace "$WORKSPACE" \
...
python3 "$ROOT_DIR/scripts/kernel_preview_rehearsal_loop.py" --validate "$REPORT_JSON"

python3 - "$REPORT_JSON" <<'PY'
import json, pathlib, sys
payload = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
assert payload["schema_version"] == "agentos-preview-rehearsal-loop.v1"
assert payload["summary"]["recommendation"] == "hold"
assert payload["summary"]["recovery_clarity"] == "watch"
assert payload["summary"]["next_action"] == "repeat_rehearsal_after_review"
assert payload["preflight"]["passed"] is True
assert pathlib.Path(payload["artifacts"]["preview_rehearsal_manifest_json"]).exists()
print("kernel preview rehearsal loop smoke: PASS")
PY
