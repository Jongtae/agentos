...
OUT_JSON="$TMP_DIR/updated-broader-preview-launch-pack.json"
python3 scripts/kernel_updated_broader_preview_launch_pack.py \
  --workspace "$WORKSPACE" \
...
python3 scripts/kernel_updated_broader_preview_launch_pack.py --validate "$OUT_JSON"

python3 - "$OUT_JSON" <<'PY'
import json, pathlib, sys
payload = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding='utf-8'))
assert payload["schema_version"] == "agentos-updated-broader-preview-launch-pack.v1"
assert payload["summary"]["candidate_state"] in {"candidate_watch", "broader_preview_candidate"}
assert payload["preview_posture_rescore"]["schema_version"] == "agentos-preview-posture-rescore.v1"
assert payload["prior_broader_preview_launch_pack"]["schema_version"] == "agentos-broader-preview-launch-pack.v1"
assert pathlib.Path(payload["artifacts"]["updated_broader_preview_launch_pack_json"]).exists()
print("kernel updated broader preview launch pack smoke: PASS")
PY
