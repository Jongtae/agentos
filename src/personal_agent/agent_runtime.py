...
class Capabilities:
 def __init__(self,store,adapter,config,key,job_id,record,readonly=False,network=None,document_access=True,packages=None,allowed_tools=None,document_context=False,public_page_scope=None,memory_approval=None,inherited_provenance=(),calendar=None,calendar_owner=None,memory_request=None,current_packages=None,lookup_sources=None,lookup_hint='',delegated=False,inherited_excluded=(),budget=None,judgments=None):
...
def path_key(action,args,page=None):
 ...
 when action, target and input match on the same page digest.  The model's
 ...
 if action in PAGE_STEP_ACTIONS and page:
  return json.dumps([action,sorted((key,_normalized(value)) for key,value in args.items() if key!='effect'),page],ensure_ascii=False)
 return None
 ...
 while True:
  ...
  for call in calls:
   ...
   try:
    ...
    running={'scope':scope,'call_id':call['id'],'attempt':attempt,'host_action':action,'arguments':args}
    if kind:
    ...
    result=capabilities.memo[cache_key];ran=True
