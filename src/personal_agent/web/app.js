...
function showSubscriptionEngines(subscription){
 let box=$('subscription-engines');if(!box){box=element('section');box.id='subscription-engines';box.className='subscription-engines';const title=element('h3','Telegram용 구독 엔진');const help=element('p','Telegram 요청을 처리할 CLI입니다. 웹·파일 작업의 OpenAI API 연결과는 별개입니다. Codex 또는 Claude Code에 공식 로그인한 경우에만 연결하세요.');box.append(title,help);$('settings-panel').prepend(box);}
 for(const old of [...box.querySelectorAll('.subscription-engine,.settings-current')])old.remove();
 const selected=subscription?.selected||'';if(selected){const current=(subscription.engines||[]).find(engine=>engine.id===selected);box.append(element('p',`현재 선택: ${current?.name||selected} · 바꾸려면 아래의 다른 엔진 연결 버튼을 누르세요.`,'settings-current'));}
 for(const engine of subscription?.engines||[]){const card=element('div',undefined,'subscription-engine');card.append(element('strong',engine.name));if(engine.connected){card.classList.add('active');card.append(element('p','현재 선택됨 · 공식 로그인 여부는 사용자 확인으로만 기록됩니다.'));box.append(card);continue;}if(!engine.installed){const p=element('p',`CLI를 찾지 못했습니다. 공식 설치 안내에서 설치한 뒤 ${engine.login_command} 명령으로 로그인하세요. `);const link=element('a','공식 안내 ↗');link.href=engine.login_url;link.target='_blank';link.rel='noreferrer noopener';p.append(link);card.append(p);box.append(card);continue;}card.append(element('p',selected?`현재 ${selected} 대신 ${engine.name}을 사용하려면 먼저 공식 로그인하세요.`:`터미널에서 ${engine.login_command}을 실행해 공식 로그인 후 연결하세요.`));const button=element('button',selected?`${engine.name}으로 전환`:`${engine.name} 로그인 완료 · 연결`);button.type='button';button.onclick=()=>busy(button,async()=>{try{await api('/api/subscription-engines/connect',{engine:engine.id,officially_authenticated:true});await refresh();}catch(e){error('global-error',e);}});card.append(button);box.append(card);}
}
...
async function refresh(){
 ...
 try{
 const home=await api('/api/home');const state=await api('/api/state');const space=await api('/api/personal-space');const guide=await api('/api/onboarding');const settings=state.settings;const model=settings.model;const tg=settings.telegram;hasModel=home.model_connected;showSubscriptionEngines(settings.subscription_engines);showOnboarding(guide);showWorkspaces(home,state);showPersonalSpace(space);showConversationSettings(settings.conversation_settings);$('home-next-action').textContent=home.next_action;const suggestion=$('workspace-suggestion');suggestion.replaceChildren();if(home.workspace_suggestion){suggestion.hidden=false;suggestion.append(document.createTextNode('이 대화를 작업공간으로 정리할까요? '));const button=element('button','작업공간 만들기');button.type='button';button.onclick=()=>{$('workspace-link').click();$('workspace-title').focus();};suggestion.append(button);}else suggestion.hidden=true;
