...
from kernel_broader_preview_cohort_operations import (
    build_broader_preview_cohort_operations,
    validate_broader_preview_cohort_operations,
)


class KernelBroaderPreviewCohortOperationsTests(unittest.TestCase):
    def test_build_broader_preview_cohort_operations_writes_expected_layout(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(payload["schema_version"], "agentos-broader-preview-cohort-operations.v1")
            self.assertEqual(payload["summary"]["delivery_scope"], "broader_preview_candidate")
            self.assertIn(payload["summary"]["operating_state"], {"active", "watch"})
            self.assertTrue(Path(payload["artifacts"]["broader_preview_cohort_operations_json"]).exists())
            self.assertEqual(validate_broader_preview_cohort_operations(payload), [])
