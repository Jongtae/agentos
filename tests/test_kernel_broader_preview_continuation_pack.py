...
from kernel_broader_preview_continuation_pack import (
    build_broader_preview_continuation_pack,
    validate_broader_preview_continuation_pack,
)


class KernelBroaderPreviewContinuationPackTests(unittest.TestCase):
    def test_build_broader_preview_continuation_pack_writes_expected_layout(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(payload["schema_version"], "agentos-broader-preview-continuation-pack.v1")
            self.assertIn(payload["summary"]["operating_health"], {"watch", "steady", "strong"})
            self.assertTrue(Path(payload["artifacts"]["broader_preview_continuation_pack_json"]).exists())
            self.assertEqual(validate_broader_preview_continuation_pack(payload), [])
