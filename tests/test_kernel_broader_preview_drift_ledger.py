...
from kernel_broader_preview_drift_ledger import (
    build_broader_preview_drift_ledger,
    validate_broader_preview_drift_ledger,
)
from kernel_broader_preview_signal_snapshot import build_broader_preview_signal_snapshot


class KernelBroaderPreviewDriftLedgerTests(unittest.TestCase):
    def test_build_broader_preview_drift_ledger_writes_expected_layout(self) -> None:
        with tempfile.TemporaryDirectory(prefix="apd-", dir="/tmp") as tmpdir:
                ...
                json.dumps(
                    {
                        ...
                        "summary": {
                            ...
                            "session_origin": "live_appliance_boot",
                            "install_validation_ok": True,
                            "audit_ok": True,
                            "diagnostics_ok": True,
                            "diagnostics_readiness_status": "ready",
                            "approval_forensic_status": "requested",
            ...
            build_broader_preview_signal_snapshot(
            ...
            self.assertEqual(payload["schema_version"], "agentos-broader-preview-drift-ledger.v1")
            self.assertIn(payload["summary"]["drift_state"], {"baseline", "improving", "steady", "regressing"})
            self.assertTrue(Path(payload["artifacts"]["broader_preview_drift_ledger_json"]).exists())
            self.assertEqual(validate_broader_preview_drift_ledger(payload), [])
