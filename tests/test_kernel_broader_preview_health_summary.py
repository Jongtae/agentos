...
from kernel_broader_preview_health_summary import (
    build_broader_preview_health_summary,
    validate_broader_preview_health_summary,
)


class KernelBroaderPreviewHealthSummaryTests(unittest.TestCase):
    def test_build_broader_preview_health_summary_writes_expected_layout(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(payload["schema_version"], "agentos-broader-preview-health-summary.v1")
            self.assertIn(payload["summary"]["operating_health"], {"watch", "steady", "strong"})
            self.assertTrue(Path(payload["artifacts"]["broader_preview_health_summary_manifest_json"]).exists())
            self.assertEqual(validate_broader_preview_health_summary(payload), [])
