...
from kernel_broader_preview_issue_ledger import (
    build_broader_preview_issue_ledger,
    validate_broader_preview_issue_ledger,
)


class KernelBroaderPreviewIssueLedgerTests(unittest.TestCase):
    def test_build_broader_preview_issue_ledger_writes_expected_layout(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(payload["schema_version"], "agentos-broader-preview-issue-ledger.v1")
            self.assertIn(payload["summary"]["operating_decision"], {"pause_risk_present", "continue_with_watch_items", "expand_ready"})
            self.assertTrue(Path(payload["artifacts"]["broader_preview_issue_ledger_manifest_json"]).exists())
            self.assertEqual(validate_broader_preview_issue_ledger(payload), [])
