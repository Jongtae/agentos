...
from kernel_broader_preview_readiness_scoreboard import (
    build_broader_preview_readiness_scoreboard,
    validate_broader_preview_readiness_scoreboard,
)


class KernelBroaderPreviewReadinessScoreboardTests(unittest.TestCase):
    def test_build_broader_preview_readiness_scoreboard_writes_expected_layout(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            ledger_root = artifacts / "public-preview" / "broader-preview-readiness-scoreboard" / "iteration" / "limited-preview-iteration-ledger"
            ...
            self.assertEqual(payload["schema_version"], "agentos-broader-preview-readiness-scoreboard.v1")
            ...
            self.assertIn("recovery_clarity", payload["summary"]["blocked_reasons"])
            self.assertTrue(Path(payload["artifacts"]["broader_preview_readiness_scoreboard_manifest_json"]).exists())
            self.assertEqual(validate_broader_preview_readiness_scoreboard(payload), [])
