...
from kernel_broader_preview_weekly_summary import (
    build_broader_preview_weekly_summary,
    validate_broader_preview_weekly_summary,
)
from kernel_broader_preview_signal_snapshot import build_broader_preview_signal_snapshot


class KernelBroaderPreviewWeeklySummaryTests(unittest.TestCase):
    def test_build_broader_preview_weekly_summary_writes_expected_layout(self) -> None:
        with tempfile.TemporaryDirectory(prefix="apw-", dir="/tmp") as tmpdir:
            ...
            self.assertEqual(payload["schema_version"], "agentos-broader-preview-weekly-summary.v1")
            self.assertIn(payload["summary"]["weekly_posture"], {"watch", "stable"})
            self.assertTrue(Path(payload["artifacts"]["broader_preview_weekly_summary_json"]).exists())
            self.assertEqual(validate_broader_preview_weekly_summary(payload), [])
