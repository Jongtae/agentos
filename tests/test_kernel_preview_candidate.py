...
from kernel_preview_candidate import build_preview_candidate, validate_preview_candidate


class KernelPreviewCandidateTests(unittest.TestCase):
    def test_build_preview_candidate_writes_expected_layout(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(payload["schema_version"], "agentos-preview-candidate.v1")
            self.assertTrue(Path(payload["artifacts"]["preview_candidate_manifest_json"]).exists())
            ...
            self.assertEqual(validate_preview_candidate(payload), [])
