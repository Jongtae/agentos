...
from kernel_preview_posture_rescore import build_preview_posture_rescore, validate_preview_posture_rescore


class KernelPreviewPostureRescoreTests(unittest.TestCase):
    def test_build_preview_posture_rescore_writes_expected_layout(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            payload = build_preview_posture_rescore(
                ...
                snapshot_label="rescore",
            )

            self.assertEqual(payload["schema_version"], "agentos-preview-posture-rescore.v1")
            self.assertTrue(Path(payload["artifacts"]["preview_posture_rescore_manifest_json"]).exists())
            self.assertIn(payload["summary"]["candidate_state"], {"candidate_watch", "broader_preview_candidate"})
            self.assertEqual(validate_preview_posture_rescore(payload), [])
