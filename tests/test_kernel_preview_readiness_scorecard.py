...
from kernel_preview_readiness_scorecard import (
    build_preview_readiness_scorecard,
    validate_preview_readiness_scorecard,
)


class KernelPreviewReadinessScorecardTests(unittest.TestCase):
    def test_build_preview_readiness_scorecard_writes_expected_layout(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(payload["schema_version"], "agentos-preview-readiness-scorecard.v1")
            self.assertTrue(Path(payload["artifacts"]["preview_readiness_scorecard_json"]).exists())
            self.assertEqual(payload["summary"]["recommendation"], "hold")
            self.assertEqual(payload["summary"]["recovery_clarity"], "watch")
            self.assertIn("recovery", payload["summary"]["ux_areas_flagged"])
            self.assertEqual(validate_preview_readiness_scorecard(payload), [])
