...
from kernel_updated_broader_preview_launch_pack import (
    build_updated_broader_preview_launch_pack,
    validate_updated_broader_preview_launch_pack,
)


class KernelUpdatedBroaderPreviewLaunchPackTests(unittest.TestCase):
    def test_build_updated_broader_preview_launch_pack_writes_expected_layout(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(payload["schema_version"], "agentos-updated-broader-preview-launch-pack.v1")
            self.assertTrue(Path(payload["artifacts"]["updated_broader_preview_launch_pack_json"]).exists())
            self.assertIn(payload["summary"]["candidate_state"], {"candidate_watch", "broader_preview_candidate"})
            self.assertEqual(validate_updated_broader_preview_launch_pack(payload), [])
