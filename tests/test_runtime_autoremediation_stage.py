...
class RuntimeAutoremediationStageTests(unittest.TestCase):
    def test_dry_run_outputs_stage_sections(self):
    ...
    def test_apply_returns_nonzero_when_batch_fails(self):
        with tempfile.TemporaryDirectory() as td:
            ws = Path(td) / "workspace"
            artifacts = ws / "artifacts"
            artifacts.mkdir(parents=True, exist_ok=True)
            ...
            payload = json.loads(proc.stdout.strip())
            self.assertNotEqual(int(payload.get("batch_exit_code", 0)), 0)
